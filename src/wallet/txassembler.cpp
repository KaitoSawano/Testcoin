    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional EDOC outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes EDOC and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::EDOC_TO_EDOC) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        if (SelectCoins(new_tx, nTargetValue, params_pegin)) {
            return std::any_of(new_tx.selected_coins.cbegin(), new_tx.selected_coins.cend(), is_edoc);
        }
    } else {
        // First try to construct a EDOC-to-EDOC transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::EDOC_ONLY;
        ...
        // If EDOC-to-EDOC fails, create a peg-out transaction
