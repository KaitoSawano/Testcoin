...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    try:
        ...
        if subprocess.run(["pgrep", "-x", "edocashd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a edocashd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
...
def check_script_prefixes():
    ...
    good_prefixes_re = re.compile("(example|feature|interface|mempool|mining|p2p|rpc|wallet|tool|edoc|mweb)_")
...
class RPCCoverage():
    ...
    commands per `edocash-cli help` (`rpc_interface.txt`).
